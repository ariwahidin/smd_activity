...
class spv_model extends CI_Model
{
    ...
    public function getDataActivity($number = null)
    {
        ...
        $sql = "SELECT t1.*,t3.fullname,t2.activity_name FROM tb_activity t1
        INNER JOIN master_activity t2 ON t1.activity = t2.id
        INNER JOIN [pk_activity].[dbo].[master_user] t3 ON t1.CreatedBy = t3.username
        WHERE t1.CreatedBy IN(SELECT '$username' UNION SELECT username FROM [pk_activity].[dbo].[master_user] WHERE spv = '$username')
        AND t1.assign IN(SELECT '$username' UNION SELECT username FROM [pk_activity].[dbo].[master_user] WHERE spv = '$username' )";
        if ($number != null) {
    ...
    public function updateKomentarIsRead($number)
    {
        ...
        read_by = '$komentator', read_time = '$date' 
        WHERE is_read = 'N'
        AND activity_number = '$number' 
        AND created_by IN (SELECT username FROM [PK_ACTIVITY].[dbo].[master_user] WHERE spv = '$komentator')";
        $query = $this->db->query($sql);
        return $query;
    }
    ...
    {
        $username = $_SESSION['username'];
        $sql = "SELECT * FROM [pk_activity].[dbo].[master_user] WHERE spv = '$username' AND [page] = 'sales'";
        $query = $this->db->query($sql);
        return $query;
    }
